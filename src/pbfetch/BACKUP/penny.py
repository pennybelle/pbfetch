...
def split_at_length(text, max_length):
    ...
    COMMAND_PATTERNS = [
        re.compile(r"RGB\(\s*([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])\s*,\s*([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])\s*,\s*([01]?[0-9][0-9]?|2[0-4][0-9]|25[0-5])\s*\)"),
        re.compile(r"RST")
        ]
    ...
    for i in range(0, text_length):
        ...
        if current_char == START_FLAG:
            ...
            if (re.fullmatch(COMMAND_PATTERNS[0], buffer, flags=0) or
                re.fullmatch(COMMAND_PATTERNS[1], buffer, flags=0)):
                print(buffer)
